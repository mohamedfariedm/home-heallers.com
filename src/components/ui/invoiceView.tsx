...
// Unique services (header display)
  ...
  const handleDownloadPDF = async () => {
    ...
    let targetElement: HTMLElement = invoiceRef.current;
    let hiddenContainer: HTMLDivElement | null = null;
    
    // Only use hidden container on mobile devices
    if (isMobile) {
    ...
    try {
      ...
      // Force a reflow to ensure layout is calculated (only on mobile)
      if (isMobile && hiddenContainer) {
      ...
      const canvas = await html2canvas(targetElement, {
        ...
        width: targetElement.scrollWidth || (isMobile ? 800 : targetElement.offsetWidth),
      ...
      // Limit canvas size to prevent memory issues (max 4000px width)
      let finalCanvas = canvas;
      if (canvas.width > 4000) {
        const maxWidth = 4000;
        const ratio = maxWidth / canvas.width;
        const maxHeight = canvas.height * ratio;
      ...
      // Convert to JPEG with compression to reduce file size and ensure compatibility
      let imgData: string;
      let imageFormat: 'JPEG' | 'PNG' = 'JPEG';
      try {
        imgData = finalCanvas.toDataURL('image/jpeg', 0.92);
        imageFormat = 'JPEG';
      } catch (jpegError) {
      ...
      // Validate image data length (should be at least a few KB for a valid image)
      if (imgData.length < 1000) {
        throw new Error('Image data is too small, generation may have failed');
      }

      console.log('Image data size:', (imgData.length / 1024).toFixed(2), 'KB', 'Format:', imageFormat);
      ...
      // Add first page
      try {
        pdf.addImage(imgData, imageFormat, 10, position, imgWidth, imgHeight);
        ...
        throw new Error('Failed to add image to PDF: ' + (imgError instanceof Error ? imgError.message : String(imgError)));
      }

      // Validate PDF before saving - check if PDF has content
      try {
      ...
      // Clean up: remove hidden container (only if it was created)
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white" dir="rtl">
      <div className="flex justify-center items-center mb-4">
        <Button 
          ...
          className="mb-4 bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isGeneratingPDF ? 'جاري إنشاء PDF...' : 'تحميل الفاتورة كـ PDF'}
        </Button>
      </div>

      <Card ref={invoiceRef} data-invoice-ref className="p-8 shadow-lg">
        {/* Header */}
