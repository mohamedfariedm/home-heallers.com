...
const doctorRoleOptions = [
  { value: 'Consultant', name: 'Consultant', label: <Text className="ms-2 font-medium">CONSULTANT</Text> },
  { value: 'Specialist', name: 'Specialist', label: <Text className="ms-2 font-medium">SPECIALIST</Text> },
  { value: 'Resident', name: 'Resident', label: <Text className="ms-2 font-medium">RESIDENT</Text> },
...
export default function FilterElement({
  ...
  const fieldProps = isMediumScreen
    ? {
  ...
  return (
    <>
      ...
      <StatusField
        ...
        value={filters['doctor_role']}
        onChange={(value: string) => updateFilter('doctor_role', value)}
        getOptionValue={(option) => option.value}
        displayValue={(selected: string) =>
          doctorRoleOptions.find((option) => option.value === selected)?.label ?? selected
        }
        {...(isMediumScreen && {
