...
export default function RadialBarChart({ className, targetPercent }: { className?: string, targetPercent?: any[] }) {
  const isMobile = useMedia('(max-width: 480px)', false);
  const data = [{name: 'Total 100%', sales: 100, fill: colors[0]}].concat(targetPercent?.map((item: any, index: number) =>( {
    name: `${item?.name} ${item?.target_percent}%`,
    sales: item?.target_percent,
    fill: colors[index%4]
  })) || defaultData)
  return (
    <WidgetCard
      title={'Category Target Contribution'}
