  ...
  return (
    <>
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['in_store_date_from'] ? new Date(filters['in_store_date_from']) : null}
            onChange={(date: any) => {
            updateFilter('in_store_date_from', formatDate(new Date(date), 'YYYY-MM-DD'));
            }}
            placeholderText="InStore Date From"
            {...(isMediumScreen && {
            inputProps: {
        ...
        />
        <DateFiled
            selected={filters['in_store_date_to'] ? new Date(filters['in_store_date_to']) : null}
            onChange={(date: any) => {
            updateFilter('in_store_date_to', formatDate(new Date(date), 'YYYY-MM-DD'));
            }}
            placeholderText="InStore Date To"
            {...(isMediumScreen && {
            inputProps: {
