...
import { Trash2, Edit, Plus, Save, X, FileText, Calculator, Receipt } from "lucide-react"
...
const InvoiceFormSchema = z.object({
  ...
  total_before_tax: z.coerce.number().min(0, "Total before tax must be non-negative"),
  ...
  status: z.enum(["قيد الانتظار", "موافق عليه", "ملغاة"]).default("قيد الانتظار"),
  notes: z.string().optional(),
  details: z.array(InvoiceDetailSchema).min(1, "At least one invoice detail is required"),
})
...
// -------------------- Component --------------------
export default function InvoiceManager({ initValues }: { initValues?: Invoice }) {
  const { closeModal } = useModal()
  const [editingDetail, setEditingDetail] = useState<InvoiceDetail | null>(null)
  ...
  const {
    ...
    defaultValues: {
      invoice_number: initValues?.invoice_number || `INV-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`,
      invoice_date: initValues?.invoice_date || new Date().toISOString().split("T")[0],
  ...
  const handleSaveDetail = () => {
    ...
    try {
      ...
      if (isAddingNew) {
        ...
        const updated = details.map((d: InvoiceDetail, i: number) => (i === editingIndex ? newDetail : d))
              ...
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormGroup title="Invoice Date">
                    <Input type="date" placeholder="Select date" {...register("invoice_date")} />
                    {errors.invoice_date && (
            ...
            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
              <CardHeader className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-t-lg">
                <CardTitle className="flex items-center justify-between text-xl">
                  ...
                  <Button onClick={handleAddDetail} size="sm" className="bg-white/20 hover:bg-white/30 border-white/30" type="button">
              ...
              <CardContent className="p-8">
                {errors.details && <p className="text-red-500 text-sm mb-4">{errors.details.message}</p>}
                <div className="space-y-6">
                  {details.map((detail: InvoiceDetail, index: number) => (
                    <Card key={String(detail.id ?? index)} className="border-l-4 border-l-emerald-500 shadow-md hover:shadow-lg transition-shadow">
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between">
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 flex-1">
                            <div className="space-y-1">
                              <p className="text-sm font-medium text-emerald-600">Customer</p>
                              <p className="font-semibold text-gray-900">{detail.customer_name}</p>
                              {detail.national_id && (
                                <p className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">ID: {detail.national_id}</p>
                              )}
                            </div>
                            <div className="space-y-1">
                            ...
                            <div className="space-y-1">
                              <p className="text-sm font-medium text-emerald-600">Doctor</p>
                              <p className="font-semibold text-gray-900">
                                {doctorsData?.data?.find((d: any) => d.id === Number(detail.doctor_id))?.name?.en ||
                                  `Dr. ID: ${detail.doctor_id}`}
                              </p>
                            </div>
                            <div className="space-y-1">
                              <p className="text-sm font-medium text-emerald-600">Pricing</p>
                              <p className="font-medium text-gray-700">{Number(detail.session_price)} ر.س × {detail.session_count}</p>
                              <p className="text-lg font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded">
                                {Number(detail.session_price) * Number(detail.session_count)} ر.س
                              </p>
                            </div>
                          </div>
                          <div className="flex gap-2 ml-4">
                            <Button onClick={() => handleEditDetail(index)} size="sm" variant="outline" className="hover:bg-blue-50" type="button">
                  ...
                  {details.length === 0 && (
                    <div className="text-center py-12 text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                      ...
                      <p className="text-lg font-medium">No invoice details added yet</p>
                      <p className="text-sm">Click &quot;Add Detail&quot; to get started with your first line item.</p>
                    </div>
                  )}
                </div>
                ...
                {editingDetail && (
                  <Card className="mt-8 border-2 border-blue-500 shadow-xl">
                    ...
                    <CardContent className="p-8 space-y-6 bg-blue-50/30">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormGroup title="Customer Name *">
                          <Input
                            value={editingDetail.customer_name}
                            onChange={(e) => handleDetailFieldChange("customer_name", e.target.value)}
                            placeholder="Enter customer name"
                            className="bg-white"
                          />
                          {detailErrors?.errors.find((err) => err.path.includes("customer_name")) && (
                            <p className="text-red-500 text-sm mt-1">
                              {detailErrors.errors.find((err) => err.path.includes("customer_name"))?.message}
                            </p>
                          )}
                        </FormGroup>
                        <FormGroup title="National ID">
                          <Input
                            ...
                            onChange={(e) => handleDetailFieldChange("national_id", e.target.value)}
                          ...
                          >
                            ...
                            {doctorsData?.data?.map((doctor: any) => (
                              <option key={doctor.id} value={doctor.id}>
                                {doctor.name?.en || doctor.name?.ar || doctor.id}
                              </option>
                            ))}
                          </select>
                          {detailErrors?.errors.find((err) => err.path.includes("doctor_id")) && (
                            <p className="text-red-500 text-sm mt-1">
                              {detailErrors.errors.find((err) => err.path.includes("doctor_id"))?.message}
                            </p>
                          )}
                        </FormGroup>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <FormGroup title="Session Price (ر.س)">
                          <Input
                            ...
                            onChange={(e) => handleDetailFieldChange("session_price", e.target.value)}
                            placeholder="Enter session price"
                            className="bg-white"
                          />
                          {detailErrors?.errors.find((err) => err.path.includes("session_price")) && (
                            <p className="text-red-500 text-sm mt-1">
                              {detailErrors.errors.find((err) => err.path.includes("session_price"))?.message}
                            </p>
                          )}
                        </FormGroup>
                        ...
                        <FormGroup title="Session Count">
                          <Input
                            ...
                            onChange={(e) => handleDetailFieldChange("session_count", e.target.value)}
                            placeholder="Enter session count"
                            className="bg-white"
                          />
                          {detailErrors?.errors.find((err) => err.path.includes("session_count")) && (
                            <p className="text-red-500 text-sm mt-1">
                              {detailErrors.errors.find((err) => err.path.includes("session_count"))?.message}
                            </p>
                          )}
                        </FormGroup>
                        ...
                        <FormGroup title="Tax Percentage *">
                          <select
                            ...
                            onChange={(e) => handleDetailFieldChange("tax_percentage", e.target.value)}
                            className="w-full border border-gray-300 rounded-lg p-3 bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          >
                          ...
                          {detailErrors?.errors.find((err) => err.path.includes("tax_percentage")) && (
                            <p className="text-red-500 text-sm mt-1">
                              {detailErrors.errors.find((err) => err.path.includes("tax_percentage"))?.message}
                            </p>
                          )}
                        </FormGroup>
                      </div>
                      ...
                      <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                        ...
                        <Button onClick={handleSaveDetail} size="lg" className="bg-blue-600 hover:bg-blue-700" type="button">
              ...
              <CardContent className="p-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                  <div className="text-center p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200">
                    <p className="text-sm font-medium text-blue-600 mb-2">Total Before Tax</p>
                    <p className="text-2xl font-bold text-blue-800">{Number(watch("total_before_tax"))} ر.س</p>
                  </div>
                  <div className="text-center p-6 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl border border-emerald-200">
                    <p className="text-sm font-medium text-emerald-600 mb-2">Tax (per-line)</p>
                    <p className="text-2xl font-bold text-emerald-800">{Number(watch("tax_total"))} ر.س</p>
                  </div>
                  <div className="text-center p-6 bg-gradient-to-br from-red-50 to-red-100 rounded-xl border border-red-200">
                    <p className="text-sm font-medium text-red-600 mb-2">Discount</p>
                    <p className="text-2xl font-bold text-red-800">-{Number(watch("discount"))} ر.س</p>
                  </div>
                  <div className="text-center p-6 bg-gradient-to-br from-purple-600 to-purple-700 text-white rounded-xl shadow-lg">
                    <p className="text-sm font-medium opacity-90 mb-2">Grand Total</p>
...
{errors.details && (
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      color={watch("status") === "موافق عليه" ? "primary" : "secondary"}
                      className={watch("status") === "موافق عليه" ? "bg-green-100 text-green-800 border-green-200" : ""}
                    >
                      {watch("status")}
                    </Badge>
                  </div>
                  <Button
                    type="submit"
                    size="lg"
                    className="px-12 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-lg"
                    disabled={isCreating || isUpdating}
                  >
                    {isCreating || isUpdating ? <Spinner size="sm" className="mr-2" /> : null}
