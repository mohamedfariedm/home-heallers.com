...
export const getColumns = ({
  ...
  },
  {
    ...
    render: (_: any, row: any) => (
      <div>
        <div className="font-medium">{row?.client?.en ?? '—'}</div>
        <div className="text-sm text-gray-500">{row?.client?.ar ?? '—'}</div>
      </div>
    ),
  },
  {
    ...
    dataIndex: 'doctor',
    key: 'doctor',
    render: (_: any, row: any) => row?.doctor?.name?.en ?? '—',
  },
  {
    title: <HeaderCell title="Service" />,
    dataIndex: 'service',
    key: 'service',
    render: (_: any, row: any) => row?.service?.en ?? '—',
  },
  {
  ...
  },
  {
    ...
    key: 'total_amount',
    render: (total_amount: string) => `$${total_amount}` ?? '—',
  },
  {
    ...
    return (
      <div className="flex flex-col gap-1 w-[200px]">
        {sessions.map((date: any, index: number) => {
          const start = new Date(date.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          const end = new Date(date.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          return (
            <div key={`${date.start_time}-${index}`} className="text-sm text-gray-700">
              <span className="font-medium">{date.time_period}</span>: {start} - {end}
            </div>
          );
        })}
      </div>
