...
export default function PasswordSettingsView({
  ...
  const TEMP_VALUE = "Empty";
  const onSubmit: SubmitHandler<PasswordFormTypes> = (data) => {
    let updatedFields: any = {};


    if (!data.title?.en?.trim()) {
      data.title.en = TEMP_VALUE;
      updatedFields.title = { ...(updatedFields.title || {}), en: "" };
    }
    if (!data.title?.ar?.trim()) {
      data.title.ar = TEMP_VALUE;
      updatedFields.title = { ...(updatedFields.title || {}), ar: "" };
    }
    if (!data.description?.en?.trim()) {
      data.description.en = TEMP_VALUE;
      updatedFields.description = { ...(updatedFields.description || {}), en: "" };
    }
    if (!data.description?.ar?.trim()) {
      data.description.ar = TEMP_VALUE;
      updatedFields.description = { ...(updatedFields.description || {}), ar: "" };
    }
    ...
    update(
      ...
      {
        onSuccess: () => {
          // إعادة تعيين الحقول التي تم تعديلها فقط، بدون التأثير على الحقول الأخرى
          setReset((prevReset) => ({
            ...prevReset,
            ...updatedFields,
          }));
