...
import { useMainCategories, useCreateMainCategory, useUpdateMainCategory, useParentMainCategories, useSupSupCategories } from '@/framework/maincategories ';
import toast from 'react-hot-toast';
import Spinner from '@/components/ui/spinner';
import { log } from 'console';
import { data } from '../logistics/shipment/details/tracking-history';
...
export default function UpdateCreateRegion({ initValues }: {
  ...
  const { data: ParentCategories, isLoading: isCategoriesLoading} = useSupSupCategories("")
  ...
  const { data: supCategories, isLoading: issupCategoriesLoading} = useParentMainCategories(Number(id))
        ...
        {isCategoriesLoading ? (
            ...
            <Form<CategoryFormInput>
                ...
                {({ register, getValues, setValue, control,watch , formState: { errors } }) => {
                                            const SelectedCategory:any = watch('grand_parent_category')
                    ...
                    <>
                        <div className="flex items-center justify-between">
                            <Title as="h4" className="font-semibold">
                                {initValues ? 'Update BU' : 'Add a new BU'}
                            </Title>
                            ...
                            <Controller
                                ...
                                render={({ field: { value, onChange } }) => (
                                    ...
                                    <SelectBox
                                        ...
                                        options={ ParentCategories?.data.categories?.map((role: any) =>{
                                        ...
                                        displayValue={(selected) =>
                                            ParentCategories?.data.categories?.find((r: any) => r.id == selected)?.name ?? ''
                                        }
                        ...
                        <FormGroup
                            ...
                            <Controller
                                ...
                                render={({ field: { value, onChange } }) => (
                                    ...
                                    <SelectBox
                                        placeholder="VCP Category"
                                        options={ supCategories?.data?.map((role: any) =>{
                                        ...
                                        displayValue={(selected) =>
                                            supCategories?.data.find((r: any) => r.id == selected)?.name ?? ''
                                        }
                                        error={errors?.parent_category?.message as string}
                                    />
                                    </>
                        ...
                        <Button
                            ...
                            className="w-full @xl:w-auto"
                        >
                            {initValues ? 'Update BU' : 'Create BU'}
                        </Button>
                        </div>
                    </>
                    );
