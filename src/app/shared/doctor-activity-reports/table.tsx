...
import { getDoctorActivityColumns } from '@/app/shared/doctor-activity-reports/columns';
import type { DoctorActivityRow } from '@/types/doctor-activity-report';

const FilterElement = dynamic(
  () => import('@/app/shared/doctor-activity-reports/filter-element'),
  { ssr: false }
);
...
export default function DoctorActivityReportsTable({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      params.delete('limit');
      if (!params.get('tab')) params.set('tab', 'doctors');
  ...
  const handleReset = () => {
    ...
    setSearchInput('');
    setPageSize(25);
    router.push(`${pathname}?tab=doctors&page=1&per_page=25`);
  };

  const columns = React.useMemo(
    () =>
      getDoctorActivityColumns({
        sortConfig,
        onHeaderCellClick,
      }),
